...
def main():
    ...
    print("\n=== STEP 1: Trajectory Story Generation (Gastouni) ===")
    story = generate_story_from_trajectory(api_key, trajectory_csv)
    
    # Αποθήκευση story
    os.makedirs(config.STORY_OUTPUT, exist_ok=True)
    story_file = os.path.join(config.STORY_OUTPUT, "gastouni_1_trajectory_story.txt")
    ...
    # Αποθήκευση POIs
    pois_file = os.path.join(config.STORY_OUTPUT, "gastouni_1_trajectory_pois.json")
    ...
    output_map = os.path.join(config.STORY_OUTPUT, "gastouni_1_trajectory_map.png")
    plot_trajectory_with_pois(trajectory_csv, pois, output_map, city="Gastouni")
