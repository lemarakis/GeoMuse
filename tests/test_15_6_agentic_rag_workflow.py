...
def main():
    trajectory_csv = os.path.join(config.BASE_DIR, "sstd2025", "zakinthos-1.csv")
    image_path = os.path.join(config.STORY_OUTPUT, "zakinthos_1_trajectory_map.png")
    output_json = os.path.join(config.STORY_OUTPUT, "zakinthos_1_agentic_rag_story.json")
    ...
    print("=== Sampling Trajectory (Zakinthos) ===")
    ...
    print("\n=== Starting Agentic RAG Workflow (Zakinthos) ===")
    ...
    if final_story:
        ...
        if pois_data:
            ...
            plot_trajectory_deviation(trajectory_csv, pois_data, map_filename, city="Zakinthos", use_llm_coords=True)
