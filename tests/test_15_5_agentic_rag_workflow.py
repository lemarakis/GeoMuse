...
def main():
    trajectory_csv = os.path.join(config.BASE_DIR, "sstd2025", "ouranoupoli-1.csv")
    image_path = os.path.join(config.STORY_OUTPUT, "ouranoupoli_1_trajectory_map.png")
    output_json = os.path.join(config.STORY_OUTPUT, "ouranoupoli_1_agentic_rag_story.json")
    
    if not os.path.exists(trajectory_csv):
        print(f"Error: Missing trajectory {trajectory_csv}.")
        return
    ...
    print("=== Sampling Trajectory (Ouranoupoli) ===")
    ...
    for lat, lon in sampled_points:
        pois = get_nearby_pois(lat, lon, google_api_key, radius=100, types=None, max_results=2)
        for p in pois:
    ...
    print("\n=== Starting Agentic RAG Workflow (Ouranoupoli) ===")
    ...
    if final_story:
        ...
        if pois_data:
            ...
            plot_trajectory_deviation(trajectory_csv, pois_data, map_filename, city="Ouranoupoli", use_llm_coords=True)
