...
def main():
    trajectory_csv = os.path.join(config.BASE_DIR, "sstd2025", "trajectory_2.csv")
    image_path = os.path.join(config.STORY_OUTPUT, "athens_trajectory_2_map.png")
    output_json = os.path.join(config.STORY_OUTPUT, "athens_agentic_rag_story.json")
    
    if not os.path.exists(trajectory_csv) or not os.path.exists(image_path):
        print("Error: Missing trajectory or map image. Run test_05_2 first.")
        return
    ...
    print("=== Sampling Trajectory (Athens) ===")
    ...
    print("\n=== Starting Agentic RAG Workflow (Athens) ===")
    ...
    if final_story:
        ...
        if pois_data:
            ...
            plot_trajectory_deviation(trajectory_csv, pois_data, map_filename, city="Athens", use_llm_coords=True)
