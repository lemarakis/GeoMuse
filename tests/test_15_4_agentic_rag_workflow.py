...
def main():
    trajectory_csv = os.path.join(config.BASE_DIR, "sstd2025", "nafplio-1.csv")
    image_path = os.path.join(config.STORY_OUTPUT, "nafplio_1_trajectory_map.png")
    output_json = os.path.join(config.STORY_OUTPUT, "nafplio_1_agentic_rag_story.json")
    ...
    print("=== Sampling Trajectory (Nafplio) ===")
    ...
    print("\n=== Starting Agentic RAG Workflow (Nafplio) ===")
    ...
    if final_story:
        ...
        if pois_data:
            ...
            plot_trajectory_deviation(trajectory_csv, pois_data, map_filename, city="Nafplio", use_llm_coords=True)
