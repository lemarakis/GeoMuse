...
def main():
    trajectory_csv = os.path.join(config.BASE_DIR, "sstd2025", "santorini-1.csv")
    image_path = os.path.join(config.STORY_OUTPUT, "santorini_1_trajectory_map.png")
    output_json = os.path.join(config.STORY_OUTPUT, "santorini_1_agentic_rag_story.json")
    ...
    print("=== Sampling Trajectory (Santorini) ===")
    ...
    print("\n=== Starting Agentic RAG Workflow (Santorini) ===")
    ...
    if final_story:
        ...
        if pois_data:
            ...
            plot_trajectory_deviation(trajectory_csv, pois_data, map_filename, city="Santorini", use_llm_coords=True)
