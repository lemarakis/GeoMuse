...
def main():
    trajectory_csv = os.path.join(config.BASE_DIR, "sstd2025", "Gastouni-1.csv")
    image_path = os.path.join(config.STORY_OUTPUT, "gastouni_1_trajectory_map.png")
    output_json = os.path.join(config.STORY_OUTPUT, "gastouni_1_agentic_rag_story.json")
    ...
    print("=== Sampling Trajectory (Gastouni) ===")
    ...
    print("\n=== Starting Agentic RAG Workflow (Gastouni) ===")
    ...
    if final_story:
        ...
        if pois_data:
            ...
            plot_trajectory_deviation(trajectory_csv, pois_data, map_filename, city="Gastouni", use_llm_coords=True)
