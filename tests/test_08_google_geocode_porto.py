...
def main():
    ...
    city = "Porto"
    country = "Portugal"
    pois_file = os.path.join(config.STORY_OUTPUT, "porto_fake_pois_unbiased.json")
    
    if not os.path.exists(pois_file):
        print(f"Error: {pois_file} not found. Run test_06 first.")
        return
    ...
    if comparison_data:
        comparison_file = os.path.join(config.STORY_OUTPUT, "porto_google_comparison.json")
        with open(comparison_file, "w", encoding="utf-8") as f:
            json.dump(comparison_data, f, ensure_ascii=False, indent=4)
        print(f"\nComparison data saved to: {comparison_file}")
        
        plot_file = os.path.join(config.STORY_OUTPUT, "porto_google_comparison_map.png")
